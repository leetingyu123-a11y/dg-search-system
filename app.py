...
# Handle F5 execution lag by giving browser 0.5s buffer to return cookies to Python
...
# Securely read cookies from the browser
...
# ==============================================================================
# SETTINGS: Dedicated Gmail account for OTP verification only
# ==============================================================================
...
def send_otp_email(to_email, otp_code):
    """ Send secure OTP verification code email """
    mail_content = (
        f"Dear Colleague,\n\n"
...
# ==============================================================================
# Core "Carrier DG Restriction Query System" Starts Here
# ==============================================================================
...
st.markdown("""
    ...
    .remark-line {
        font-size: 20px !important; line-height: 1.6; color: #1e293b; font-weight: 500;
...
def handle_search():
    ...
    if cls_val or un_val:
        ...
        if history_display not in [h["display"] for h in st.session_state.history_list]:
            ...
            if len(st.session_state.history_list) > 10: st.session_state.history_list.pop()
    else:
        st.session_state.search_submitted = False
        st.session_state.last_query = None
...
def parse_sheet_version(sheet_name):
    ...
    return sheet_name.upper(), "Ver: Latest"
    ...
    try:
        ...
        if raw_master_df is not None:
            try:
                ...
                st.warning(f"⚠️ Warning: imdg_master.xlsx failed to load. Error: {e}")
        ...
        if st.session_state.search_submitted and st.session_state.last_query is not None:
            ...
            if input_un in MULTI_CLASS_UNS and not final_class:
                st.error(f"❌ INTERCEPT WARNING: UN {input_un} contains multiple classifications. Enter 'Class' field!")
                is_valid_input = False
                
            if is_valid_input and not input_un and not final_class:
                st.warning("⚠️ Action Required: Enter a UN Number or a Class/Division.")
                is_valid_input = False
                
            if is_valid_input and final_class and final_class != 'ALL':
                cleaned_num_str = clean_class_string(final_class)
                try:
                    if float(cleaned_num_str) < 1.0 or float(cleaned_num_str) >= 10.0:
                        st.error("❌ Input Error: Classes range from 1 to 9.")
                        is_valid_input = False
                except ValueError:
                    st.error("⚠️ Invalid Format: Class must be numeric numbers.")
                    is_valid_input = False
            ...
            if is_valid_input and input_un and has_master:
                ...
                if un_exists.empty:
                    ...
                    # 📌 INTEGRATED B-PLAN MODULE: 4-Digit Validation & Automated Google Sheets Logger (地雷補磚器)
                    raw_un_string = query_data["un"].strip() if query_data["un"] else ""
                    if len(raw_un_string) == 4 and raw_un_string.isdigit():
                        alert_key = f"sheet_logged_{input_un}"
                        if alert_key not in st.session_state:
                            # Trigger backend cloud sync script
                            log_missed_un_to_sheets(st.session_state.get('user_email', 'Unknown User'), input_un)
            ...
            if is_valid_input and matched_master_records:
                ...
                for record in matched_master_records:
                    ...
                    for sheet_name, display_label in search_targets:
                        ...
                        if 'UN' not in col_mapping or 'Class' not in col_mapping or 'Prohibited' not in col_mapping: continue
                        ...
                        for _, g_row in global_lines.iterrows():
                            ...
                            sub_risk_hit = False
                            hit_subrisk_val = ""
                            if master_subrisk_list and g_row['Clean_Class']:
                                for sr in master_subrisk_list:
                                    if sr != "P" and is_class_matching(sr, g_row['Clean_Class'], exact_mode=is_exact):
                                        sub_risk_hit, hit_subrisk_val = True, sr
                                        break
                                ...
                                for r_col in remark_cols:
                                    ...
                                    if r_val and r_val.lower() != 'nan' and r_val != '':
                                        if g_row['Clean_Class'] == 'ALL':
                                            ...
                                            label = f"Main Class {g_row['Clean_Class']} Policy" if main_class_hit else f"Sub Risk '{hit_subrisk_val}' Restriction"
                                            if r_val not in [c["text"] for c in specific_dg_list]: specific_dg_list.append({"col_name": label, "text": r_val})

                        is_any_row_prohibited, is_any_row_remarked = False, False
                        for row in carrier_matched_rows:
                            if any(k in str(row['Clean_Prohibited']).strip().upper() for k in ["🔴", "禁收", "YES", "PROHIBITED"]): is_any_row_prohibited = True
                            if any(k in str(row['Clean_HasRemark']).strip().upper() for k in ["🟡", "YES", "TRUE"]): is_any_row_remarked = True

                        un_display = f"UN {input_un} (Class {current_class})" if input_un else f"Class {current_class} Universal Policy"
                        carrier_payload = {"carrier_name": carrier_clean_name, "version_tag": version_tag, "un_display": un_display, "specific_dg_list": specific_dg_list, "collapsed_list": collapsed_list, "carrier_matched_rows": carrier_matched_rows}
                            ...
                            st.markdown(f"""
                                <div class="partner-card" style="border-left-color: {item['border_color']}; margin-bottom: 5px;">
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <span class="partner-title">{item['carrier_name']} <span class="version-badge">{item['version_tag']}</span><div style="font-size: 13px; color: #64748b; font-weight: normal; margin-top: 4px;">Ref: {item['un_display']}</div></span>
                            ...
                            if not item['carrier_matched_rows'] and not item['specific_dg_list']:
                                ...
                                if item['collapsed_list']:
                                    with st.expander(f"📄 View Global / Universal DG Policies ({len(item['collapsed_list'])} Items)", expanded=False):
                                        st.markdown(f'<div class="remark-box">' + "".join([f'<div class="collapsed-header">📌 {"Universal DG Policy " if idx == 0 else ""}{rem["num"]}. </div><div class="remark-line">{rem["text"]}</div>' for idx, rem in enumerate(item['collapsed_list'])]) + '</div>', unsafe_allow_html=True)
                            st.markdown("<br>", unsafe_allow_html=True)
